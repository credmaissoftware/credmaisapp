...
serve(async (req) => {
  ...
  try {
    ...
    const {
      ...
      purpose?: "manual" | "collection" | "service_followup";
    };
    ...
    if (action === "mark_resolved") {
      ...
      if (error) throw new Error("resolve_persist_failed");
    ...
    if (schedule_for) {
      ...
      if (error) throw new Error("schedule_persist_failed");
    ...
    if (hasMedia) {
      ...
      if (!sendRes.ok) {
        await sendRes.body?.cancel();
        return new Response(JSON.stringify({ error: "media_send_failed" }), { status: 502, headers: corsHeaders });
      }
      ...
      if (!sendRes.ok) {
        await sendRes.body?.cancel();
        return new Response(JSON.stringify({ error: "send_failed" }), { status: 502, headers: corsHeaders });
      }
    }
